...
class CheckProjectSchoolController extends Controller
{
    ...
    public function rejectProject(Request $request)
    {
        ...
        if($project->save())
            return redirect('/teacher/createProject/checkProjectSchool')->with('successMsg', '选题申请退回成功!');
        else
            return redirect('/teacher/createProject/checkProjectSchool')->with('failureMsg', '选题申请退回失败!');
    }
}